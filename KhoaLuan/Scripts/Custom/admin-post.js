...
$(document).ready(function () {
    $('.post-row').click(function () {
        $('#extensionModal').html("Không có tiện nghi");
        postId = $(this).attr('id');
        accountID = $(this).attr("accID");
        motelID = $('#motelID' + postId).html();
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success == true) {
                    $('#showCriteria').html(response.result);
                } else {
                    $('#showCriteria').html("Không có tiện nghi");
                }
            }
        });
...
$('#btnSubmitDeletePost').click(function () {
    $('#btnCancelSendMessage').trigger('click');
    var message = $('#feedback').val();
    var postTitle = $('#title' + postId).html();
    $.ajax({
        async: true,
        type: 'POST',
        ...
        success: function (response) {
            if (response.result == true) {
                $('#' + postId).remove();
                $('#closeModalPostInfo').trigger('click');
                $('#contentNotification').html('Đã xoá bài viết: "' + postTitle +'"');
                $('#notificationModal').modal('show');
            }
        }
    });
